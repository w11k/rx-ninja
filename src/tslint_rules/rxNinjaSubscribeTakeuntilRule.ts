...
export class Rule extends Lint.Rules.TypedRule {
    ...
    applyWithProgram(sourceFile: ts.SourceFile, program: ts.Program): Lint.RuleFailure[] {
        ...
        config.finalizer = config.finalizer ?? DEFAULT_CONFIG.finalizer;
        config.fileSuffix = config.fileSuffix ?? DEFAULT_CONFIG.fileSuffix;
        config.excludedFileSuffix = config.excludedFileSuffix ?? DEFAULT_CONFIG.excludedFileSuffix;

        const included = config.fileSuffix.find(suffix => sourceFile.fileName.endsWith(suffix)) !== undefined;
        const excluded = config.excludedFileSuffix.find(suffix => sourceFile.fileName.endsWith(suffix)) !== undefined;
